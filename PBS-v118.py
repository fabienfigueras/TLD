...
class Amo:
    def __init__(self,F_Amo_Brand="Home Made Amo",F_Amo_Model="Home Made Amo Model",F_BC_G1_a=float(-1),F_BC_G7_a=float(-7)):
    ...
    def show(self):
        print("Amo Brand : ",self.Amo_Brand,"Amo Model :",self.Amo_Model," BC G1 :",self.BC_G1_a," BC G7 : ",self.BC_G7_a)
...
class Impact:
    def __init__(self,iX=0,iY=0,iZ=0,iVx=0,iVy=0,iVz=0):
        self.X=float(iX)
        self.Y=float(iY)
        self.Z=float(iZ)
        self.Vx=float(iVx)
...
print(" Where param16 is the option to allow calculation of Abacus [Y/N] ")
print("")
...
#    print("Data from file :",FNmontage)
    #print("File SightHeight (mm): ",float(DL[0]),"File Fixed Angle (mRAD):",float(DL[1]))
...
#    print("Data from file :",FNrifle)
    #print("File Brand : ",DL[0],"File Model :",DL[1],"File Caliber (inch) :",float(DL[2]),"File Twist (inch) 1:",float(DL[3]),"File Twist (R/L) :",DL[4])
...
#    print("Data from file :",FNamo)
    #print("File Amo Brand : ",DL[0],"File Amo Model :",DL[1],"File BC_G1 :",float(DL[2]),"File BC_G7 :",float(DL[3]))
    amo.Amo_Brand=DL[0]
    amo.Amo_Model=DL[1]
...
#    print("Data from file :",FNbullet)
    #print("File Bullet Brand : ",DL[0],"File Bullet Model :",DL[1],"File Bullet Diameter (inch) : ",float(DL[2]),"File Bullet Mass (gr) :",float(DL[3]),"File Bullet Length (cm) :",float(DL[4]),"File Muzzle Speed (m/s) :",float(DL[5]))
...
#    print("Data from file :",FNzero)
    #print("File Zero Distance (m)", float(DL[0]), "File Error tolerance (m) ", float(DL[1]))
    #print("File Zero Altitude (m)", float(DL[2]), "File Zero Pressure (Pa) ", float(DL[3]))
    #print("File Zero Temperature (°C)", float(DL[4]), "File Zero Relative Humidity (%) ", float(DL[5])*100)
    #print("File Zero Angle (deg)",float(DL[6]))
...
#    print("Data from file :",FNenv)
    #print("File Altitude (m)", float(DL[0]), "File Pressure (Pa) ", float(DL[1]))
    #print("File Temperature (°C)", float(DL[2]), "File Relative Humidity (%) ", float(DL[3])*100)
...
# Print CSV File (Not used so far i.e v1.16)
def printCSV( fn ):
 print("================")
 print("File Name : ",fn)
 fh = open(fn,"r")
...
def Zeroing(D_zero,Prec):
    global h,bullet,NS,Az_d,Az,Co,montage,rifle,Shoot_Atm,Gamma,ICAO_Atm,Omega,SoundSpeed_ICAO,tph,Sg,zero

    Alpha_d=(100/3)

    PRS_Solver(D_zero,h,bullet.MuzzleSpeed,NS,Az_d,Az,Co,bullet.m_gr,montage.SightHeight,rifle.Twist,rifle.T_cal,rifle.Twist_C,rifle.Rifle_Brand,rifle.Rifle_Model,bullet.d_i,bullet.bl_cal,0,Shoot_Atm.Alt,Gamma,ICAO_Atm.Alt,ICAO_Atm.P,ICAO_Atm.T_C,ICAO_Atm.T_K,ICAO_Atm.RH,ICAO_Atm.Rho,ICAO_Atm.PVS,ICAO_Atm.PV,ICAO_Atm.TV,Shoot_Atm.P,Shoot_Atm.T_C,Shoot_Atm.T_K,Shoot_Atm.RH,Shoot_Atm.Rho,Shoot_Atm.PVS,Shoot_Atm.PV,Shoot_Atm.TV,Omega,SoundSpeed_ICAO)
    ...
    Alpha_RAD=float(-math.atan(Ytph/D_zero))

    Alpha_d=float(Alpha_RAD*180/math.pi)
...
def Fx(t,Vx,Vy,Vz,Vmoy,Ox,Oy,Oz,LCo,k,m,g):
    global Wind_Range

    if LCo == "N":
        Fx=float((-k/m)*abs(Vmoy)*Vx)
    else:
#
        Fx=float((-k/m)*abs(Vmoy)*Vx-2*(Oy*Vz-Oz*Vy))
# G Klimi        Fx=float((-k/m)*abs(Vmoy)*Vx-2*(-Oz*Vy))
    return(Fx)
...
#        Fy=float((-k/m)*abs(V)*Vy)
    else:
#
        Fy=float((-k/m)*abs(V)*Vy-g-2*(-(Ox*Vz-Oz*Vx)))
# G Klimi        Fy=float((-k/m)*abs(V)*Vy-g-2*(-(-Oz*Vx)))
...
def RK(D_Tir,h,MuzzleSpeed,NS,Az_d,Az,Co,m_gr,SightHeight,Twist,T_cal,Twist_C,Rifle_Brand,Rifle_Model,d_i,bl_cal,alpha_d,Alt,Gamma,Alt_ICAO,P_ICAO,T_C_ICAO,T_K_ICAO,RH_ICAO,Rho_ICAO,PVS_ICAO,PV_ICAO,TV_ICAO,P,T_C,T_K,RH,Rho,PVS,PV,TV,Omega,SoundSpeed_ICAO):
    ...
    global E_c,W_c,tph,Vxtph,Vytph,Vztph,Sg,Shoot_Wind,bullet,Lat_D,Vmod,Xtph,Ytph,Ztph,Arrow,Y_max,XY_max,TY_max,BC_Gx,BC_Go,Shoot_Wind,impact
    ...
    Vz=float(0)
#    Vy=float(bullet.V0*math.sin(alpha))+Shoot_Wind.Wsy
#    Vz=float(0)+Shoot_Wind.Wsz
#    print("Vx : ",Vx,"Vy : ",Vy,"Vz : ",Vz)
    ...
    #Caclulationg BC_Gx using Impedance Coefficient (J)
    ...
    impact.Vy=Vy
    impact.Vz=Vz
#    print("impact Vx : ",impact.Vx,"impact Vy : ",impact.Vy,"impact Vz : ",impact.Vz)
    impact.E_a=alpha
    impact.W_a=float(0)
    impact.update()
#    impact.show()
...
def PRS_Solver(D_Tir,h,MuzzleSpeed,NS,Az_d,Az,Co,m_gr,SightHeight,Twist,T_cal,Twist_C,Rifle_Brand,Rifle_Model,d_i,bl_cal,alpha_d,Alt,Gamma,Alt_ICAO,P_ICAO,T_C_ICAO,T_K_ICAO,RH_ICAO,Rho_ICAO,PVS_ICAO,PV_ICAO,TV_ICAO,P,T_C,T_K,RH,Rho,PVS,PV,TV,Omega,SoundSpeed_ICAO):
    ...
    global H_Angle_d,H_Angle,Ytph,tph,Vmod,SD,Sg,Shoot_Wind,bullet,Ytph_HA,impact,impact_HA,D_Tir_HA,Arrow
    ...
    if H_Angle_d == 0:
        ...
        print("shoot with Horizontal Angle (deg) :",H_Angle_d)
        print("Solving Balistic for corrected distance (m) :",round(D_Tir_HA,2))
...
if Zero_C == "Y":
    ...
    print("======================")
print("Alpha(0) used (deg) : ",round(zero.alpha_d,6),"Windage correction used (cm)",round(zero.wind_cm,6))

print("====================================================================================")
print("Ballistic differential equations being solved numerically using Ruge-Kutta Method...")
print("====================================================================================")

# Simulation without Coriollis
Co_B=Co
Co="N"

print("Simulation without Coriollis ")
...
if Co=="Y":
    print("Simulation with Coriollis ")

    PRS_Solver(D_Tir,h,bullet.MuzzleSpeed,NS,Az_d,Az,Co,bullet.m_gr,montage.SightHeight,rifle.Twist,rifle.T_cal,rifle.Twist_C,rifle.Rifle_Brand,rifle.Rifle_Model,bullet.d_i,bullet.bl_cal,zero.alpha_d,Shoot_Atm.Alt,Gamma,ICAO_Atm.Alt,ICAO_Atm.P,ICAO_Atm.T_C,ICAO_Atm.T_K,ICAO_Atm.RH,ICAO_Atm.Rho,ICAO_Atm.PVS,ICAO_Atm.PV,ICAO_Atm.TV,Shoot_Atm.P,Shoot_Atm.T_C,Shoot_Atm.T_K,Shoot_Atm.RH,Shoot_Atm.Rho,Shoot_Atm.PVS,Shoot_Atm.PV,Shoot_Atm.TV,Omega,SoundSpeed_ICAO)

#    impact.show()
    impact_Delta = Impact()
    #updating impact Delata

    impact_Delta.X=impact_NoC.X-impact.X
    impact_Delta.Y=impact_NoC.Y-impact.Y
    impact_Delta.Z=impact_NoC.Z-impact.Z

    impact_Delta.Vx=impact_NoC.Vx-impact.Vx
#    print("Vx Delta :",impact_Delta.Vx,"Vx No C :",impact_NoC.Vx,"Vx C :",impact.Vx)
    impact_Delta.Vy=impact_NoC.Vy-impact.Vy
    impact_Delta.Vz=impact_NoC.Vz-impact.Vz

    impact_Delta.E_a=impact_NoC.E_a-impact.E_a
    impact_Delta.W_a=impact_NoC.W_a-impact.W_a
    impact_Delta.update()
#    impact_Delta.show()
...
print("Time of Flight (s) :",round(tph,3))
...
if H_Angle_d == 0:
    print("Calculated bullet Impact parameters (With only Drag and Gravity influences without Coriolis and for horizontal shooting) ")

    print("Calculated Impact Speed Module |V| (m/s) :",round(impact_NoC.V,3))

#    print("Calculated X impact position (m) :",round(impact_NoC.X,3))
#    print("Calculated Y impact position (m) :",round(impact_NoC.Y,3))
    print("Calculated Y impact position with Horizontal Angle = 0 (m) :",round(impact_NoC.Y,3)," (cm) :",round(impact_NoC.Y*100,3))
    Y_a_NoC=float((math.atan(impact_NoC.Y/D_Tir)*1000))

else:
    print("Calculated bullet Impact parameters (With only Drag and Gravity influences without Coriolis and for NON horizontal shooting) ")
...
#    print("Calculated Y position corrected for Horizontal Angle (m) :",round(Ytph_HA,3)," (cm) :",round(Ytph_HA*100,3))

    print("Calculated impact Y position corrected for Horizontal Angle (m) :",round(impact_HA.Y,3)," (cm) :",round(impact_HA.Y*100,3))
    Y_Drop_HA=impact_HA.Y

    Y_a_NoC=float((math.atan(impact_HA.Y/D_Tir)*1000))

print("Calculated Z impact position (m) :",round(impact_NoC.Z,3))

print("Calculated Y impact Angle No Coriolis (mRAD) :",round(Y_a_NoC,3))

print("Elevation to be applied due to gravity drag and No Coriolis (clicks) :",round(-Y_a_NoC/c_mRAD,1))
...
E_c_NoC=float(-Y_a_NoC/c_mRAD)
W_c_NoC=float(W_SpinDrift)

print("=========== CORRECTIONS TO BE APPLIED WITHOUT OPTION =================")

print("Elevation (gravity, drag but Coriolis) to be applied (clicks) +=>Up -=>Down:",round(E_c_NoC,1))
...
if Co == "N":
    ...
    Y_a_Co=math.atan(impact.Y/D_Tir)*1000
    Y_a=Y_a_Co
    Z_Co=float(impact.Z)
...
print("Calculated Z shift due to lateral Wind (m) :",round(Shoot_Wind.WDz,5)," (cm) : ",round((ZC_SpinDrift*100),2))

#print("Calculated Z Angle due to Wind Drift (mRAD) :",round(math.atan(Shoot_Wind.WDz/D_Tir)*1000,3))
#Z_a=float((Ztph*1000)/D_Tir)

Z_a=float(math.atan(Shoot_Wind.WDz/D_Tir)*1000)
print("Calculated Z Angle due to Lateral Wind (mRAD) :",round(Z_a,3))
print("Windage to be applied due to Lateral Wind (clicks) :",round(-Z_a/c_mRAD,1))

W_c=W_c+(-Z_a/c_mRAD)
print("Calculated Z Angle due to Spin Drift (mRAD) :",round(math.atan(ZC_SpinDrift/D_Tir)*1000,3))
print("Windage to be applied due to Lateral Wind and Spin Drift (clicks) :",round(W_c,1))

if AJ == "N":
    AJE=float(0)
    print("Elevation to be applied due to gravity, drag and Coriolis (clicks) :",round(-Y_a/c_mRAD,1))
...
print("Calculated Angle along Y axis due to Aerodynamic Jump (mRAD) :",round(float(math.atan(AJE/D_Tir)*1000),2))
E_c_AJ=-(math.atan(AJE/D_Tir)*1000)/c_mRAD
print("Calculated Correction due to Aerodynamic Jump (click) :",round(E_c_AJ,1))

if AJ == "Y":
    Y_a=Y_a+float((math.atan(((AJE)/D_Tir))*1000))
    print("Angle due to gravity, drag and Aerodynamic Jump (mRAD) :",round(Y_a,3))
    print("Elevation to be applied due to gravity, drag and Aerodynamic Jump and Coriolis (clicks) :",round(-Y_a/c_mRAD,1))


#    print("Calculated Z shift due to Coriolis (m) :",round(Ztph,3)," (cm) : ",round((ZC_SpinDrift*100),2))
#    print("Calculated Z Angle due to Coriolis (mRAD) :",round(math.atan(Ztph/D_Tir)*1000,3))

E_c=float(-Y_a/c_mRAD)

#print("Calculated Z shift due to Wind Drift, Spin Drift and Coriolis (m) :",round((Shoot_Wind.WDz+ZC_SpinDrift+Ztph),2)," (cm) :",round((Shoot_Wind.WDz*100+ZC_SpinDrift*100+Ztph*100),2))

Z_a=float(math.atan(((Shoot_Wind.WDz+ZC_SpinDrift+Ztph)/D_Tir))*1000)

print("Calculated Z Angle due to Wind Drift, Spin Drift and Coriolis (mRAD) :",round(Z_a,3))
...
zero.alpha_d=zero.alpha_d+((E_c*c_mRAD)/1000)*(180/math.pi)

print("Alpha(0) corrected (deg):",round(zero.alpha_d,5))
...
if sys.argv[16] == "C":
    ...
    Ligne2="\t\t<td align=\"center\" colspan=\"8\">"+" RIFLE : "+rifle.Rifle_Brand+" "+rifle.Rifle_Model+" "+str(round(bullet.d_i,3))+" (inch) Win"+" - Rifle Bore "+rifle.Twist_C+"Twist 1:"+str(round(rifle.Twist,0))+" (inch)"+"- Sight Height : "+str(montage.SightHeight*1000)+" (mm)"+"</td>\n"
    OutputFile.write(Ligne2)
    OutputFile.write("\t</tr>\n")

    OutputFile.write("\t<tr>\n")
    Ligne200="\t\t<td align=\"center\" colspan=\"8\">"+" BULLET : "+str(round(bullet.d_i,3))+" (inch) Win"+str(int(bullet.m_gr))+"(gr) "+bullet.Bullet_Brand+" "+bullet.Bullet_Model+" - Muzzle Speed "+str(int(bullet.MuzzleSpeed))+" (m/s) in ICAO Atmosphere "+" - Ballistic Coefficient in current conditions : G1 "+str(round(BC_G1,3))+" - G7 "+str(round(BC_G7,3))+"</td>\n"
    ...
    if H_Angle_d==0:
        ...
        print("Shooting Angle correction : ",(math.atan(Delta_Y_Drop/D_Tir)*1000/c_mRAD))
        Y_a_Co=Y_a_Co+(math.atan(Delta_Y_Drop/D_Tir)*1000)

    Ligne6 = "\t\t<td align=\"center\">"+" TARGET DISTANCE (m) Include Gravity,Drag,Range Wind, Coriolis & Non-ICAO Influences"+"</td>"+"\t\t<td align=\"center\">"+str(int(D_Tir))+"</td>"+"\t\t<td align=\"center\">"+str(round((-Y_a_Co/c_mRAD),1))+"</td>"+"\t\t<td align=\"center\" colspan=\"1\">"+" BULLET MAX HEIGHT (m) "+"</td>"+"\t\t<td align=\"center\">"+ str(round(Y_max,2))+"</td>"+"\t\t<td align=\"center\" colspan=\"1\">"+" @ DISTANCE (m) "+"</td>"+"\t\t<td align=\"center\">"+ str(round(XY_max,1))+"</td>"+"\t\t<td align=\"center\" colspan=\"1\">"+" N A "+"</td\n>"

#    Total_E+=-round((math.atan(Y_Drop/D_Tir)*1000/c_mRAD),1)
    Total_E+=-round((Y_a_Co/c_mRAD),1)
    OutputFile.write(Ligne6)
    OutputFile.write("\t</tr>\n")

#    print("Drop Y without Angle :",Y_Drop)
#    print("Drop Y with Angle :",Ytph_HA)
    if H_Angle_d==0:
        Delta_Y_Drop=0
    else:
        Delta_Y_Drop=(Y_Drop-Y_Drop_HA)
#    print("Delta Y Drop :",Delta_Y_Drop)

    OutputFile.write("\t<tr>\n")
    Ligne7 = "\t\t<td align=\"center\">"+" SHOOTING ANGLE, relative to the horizontal (°) "+"</td>"+"\t\t<td align=\"center\">"+str(round(H_Angle_d,2))+"</td>"+"\t\t<td align=\"center\">"+str(round((math.atan(Delta_Y_Drop/D_Tir)*1000/c_mRAD),1))+"</td>"+"\t\t<td align=\"center\" colspan=\"4\">"+" SPIN DRIFT "+"</td>"+"\t\t<td align=\"center\">"+ str(round(W_SpinDrift,1)) +"</td\n>"
    OutputFile.write(Ligne7)
    OutputFile.write("\t</tr>\n")

    Total_E+=round((math.atan(Delta_Y_Drop/D_Tir)*1000/c_mRAD),1)
    ...
    OutputFile.write("\t<tr>\n")
    Ligne11 = "\t\t<td align=\"center\" colspan=\"2\">"+" AERODYNAMIC JUMP (Due to Range Wind)"+"</td>"+"\t\t<td align=\"center\">"+str(round((-math.atan(A_AJ/D_Tir)*1000/c_mRAD),1))+"</td>"+"\t\t<td align=\"center\" colspan=\"4\">"+"Coriolis Lateral impact"+"</td>"+"\t\t<td align=\"center\">"+str(round(-(math.atan(Z_Co/D_Tir)*1000/c_mRAD),1))+"</td\n>"
    ...
    Total_E+=round(-(math.atan(A_AJ/D_Tir)*1000/c_mRAD),1)
...
#    print("Max Y (m):",round(A_Y_max_R,2)," for distance (m) :",round(A_XY_max_R,0)," at time (s) :",round(A_TY_max_R,2))
...
#    print("Aerodynamic Jump (m) :",round(A_AJ,3)," (cm) ",round(A_AJ*100,2))
    A_AJ_R=-((A_AJ*1000/A_D_Tir_R)/c_mRAD)

#    print("Elevation to be applied due to Aerodynamic Jump (clicks) Drum +=>Up -=>Down:",round(A_AJ_R,0))

#    print("Total correction in Elevation to be applied (clicks) Drum +=>Up -=>Down:",round(A_E_R+A_AJ_R,0))
...
#    print("===================================")
    ...
    A_Wc_R=W_c
#    print("Windage to be applied due to Wind (clicks) Windage +=>Rigt -=>Left:",round(A_Wc_R,2))

#    print("Spin Drift (m) : ",round(A_SpinDrift,3),"(cm): ",round((A_SpinDrift*100),2))

    A_SpinDrift_R=-((A_SpinDrift*1000/A_D_Tir_R)/c_mRAD)
#    print("Windage to be applied due to Spin Drift (clicks) Drum +=>Up -=>Down:",round(A_SpinDrift_R,0))

#    print("Total correction in Windage to be applied (clicks) Windage +=>Rigt -=>Left:",round(A_Wc_R+A_SpinDrift_R,0))
...
#            print("vertical shooting Angle (°) ",A_H_Angle_d," (RAD)",round(A_H_Angle,5))
            A_D_Tir_HA=A_D_Tir*math.cos(A_H_Angle)
#            print("Distance for Simulation (m)",round(A_D_Tir_HA,1))
...
#            print("Path (m)",round(Ytph,3))
            Y_a=float((Ytph*1000)/A_D_Tir)
#            print("Path (mRAD)",round(Y_a,2))
            A_E=-Y_a/c_mRAD
#            print("Correction for Elevation (click)",round(A_E,1))

            if A_H_Angle_d == 0:
                A_Delta_E=0
                A_E_RVA=A_E
#                print("Correction for Elevation with angle = 0 (click)",round(A_E_RVA,1))
                ...
                Ligne4+="\t\t<th>"+str(int(A_H_Angle_d))+"</th>\n"
                Ligne5+="\t\t<td align=\"center\">"+str(int(A_Delta_E))+"</td>\n"
#            print("Delta correction for Elevation regarding Vertical Angle=0 (clicks) ",round(A_Delta_E,0))
...
#RK(D_Tir,h,bullet.MuzzleSpeed,NS,Az_d,Az,Co,bullet.m_gr,montage.SightHeight,rifle.Twist,rifle.T_cal,rifle.Twist_C,rifle.Rifle_Brand,rifle.Rifle_Model,rifle.d_i,bullet.bl_cal,zero.alpha_d,Shoot_Atm.Alt,Gamma,ICAO_Atm.Alt,ICAO_Atm.P,ICAO_Atm.T_C,ICAO_Atm.T_K,ICAO_Atm.RH,ICAO_Atm.Rho,ICAO_Atm.PVS,ICAO_Atm.PV,ICAO_Atm.TV,Shoot_Atm.P,Shoot_Atm.T_C,Shoot_Atm.T_K,Shoot_Atm.RH,Shoot_Atm.Rho,Shoot_Atm.PVS,Shoot_Atm.PV,Shoot_Atm.TV,Omega,SoundSpeed_ICAO)
...
#        print("Delta correction for Elevation regarding Pression =1013.25 hPa (clicks) ",round(A_Delta_E,0))
...
#        print("Local Temperature (°C) ",round((Shoot_Atm.T_C),2))
...
#    print("===================================")
    ...
    # Simulation for I / V hour
        ...
        A_E=-Y_a/c_mRAD
        A_Delta_E = A_E - A_E_R
        Z_a=float((Shoot_Wind.WDz+Ztph)*1000/D_Tir)
        W_c=float(-Z_a/c_mRAD)
    ...
    AbacusFile.write("\t\t<th colspan=\"16\">")
    Ligne1000="Spin Drift (click) : "+str(int(round(A_SpinDrift_R,0)))+" Maximum Y (m) : "+str(round(A_Y_max_R,2))+" At (m) : "+str(round(A_XY_max_R,0))+" Time to get there (s) : "+str(round(A_ToF_R,2))
