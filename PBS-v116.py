...
class Zero:
    def __init__(self,F_D_zero=100,F_Prec=0.001,F_alpha_d=0):
    ...
    def show(self):
        ...
        self.atm.show()
        print("Zero Angle (deg)",self.alpha_d)

    class ZAtm:
        def _init_(self,F_Alt=0,F_P=101325,F_T_C=15,F_RH=0):
...
class Atm:
    def _init_(self,F_Alt=0,F_P=101325,F_T_C=15,F_RH=0.0):
        self.Alt=float(F_Alt)
        self.P=float(F_P)
        self.T_C=float(F_T_C)
        self.T_K=float(self.T_C+273.15)
...
class Wind:
    def _init_(self,F_Ws=0,F_Wa_o=3):
        self.Ws=float(F_Ws)
...
# Check if Y >0 for a given distance (X) and Departure Angle

def TestAngle(Alpha,D_zero,Prec):
 global Ytph,h,bullet,NS,Az_d,Az,Co,Ws,Wa_o,montage,rifle,Shoot_Atm,Gamma,ICAO_Atm,Omega,SoundSpeed_ICAO,zero

 ta=0

 RK(D_zero,h,bullet.MuzzleSpeed,NS,Az_d,Az,Co,bullet.m_gr,montage.SightHeight,rifle.Twist,rifle.T_cal,rifle.Twist_C,rifle.Rifle_Brand,rifle.Rifle_Model,rifle.d_i,bullet.bl_cal,Alpha,zero.atm.Alt,Gamma,ICAO_Atm.Alt,ICAO_Atm.P,ICAO_Atm.T_C,ICAO_Atm.T_K,ICAO_Atm.RH,ICAO_Atm.Rho,ICAO_Atm.PVS,ICAO_Atm.PV,ICAO_Atm.TV,zero.atm.P,zero.atm.T_C,zero.atm.T_K,zero.atm.RH,zero.atm.Rho,zero.atm.PVS,zero.atm.PV,zero.atm.TV,Omega,SoundSpeed_ICAO)
...
if Zero_C == "Y":
    ...
    print("======================")
print("Alpha(0) used (deg) : ",round(zero.alpha_d,6))
...
print("Calculated Z shift due to Wind Drift and Spin Drift (m) :",round((Shoot_Wind.WDz+(SD/100)+Ztph),2)," (cm) :",round((Shoot_Wind.WDz*100+SD+Ztph*100),2))

Z_a=float((Shoot_Wind.WDz+(SD/100)+Ztph)*1000/D_Tir)
...
if sys.argv[16] == "N":
    ...
    Total_E+=-(math.atan(Y_Drop/D_Tir)*1000/0.1)
...
#    print("Delta Y Drop :",Delta_Y_Drop)
    ...
    Total_E+=(math.atan(Delta_Y_Drop/D_Tir)*1000/0.1)
    Total_W+=W_SpinDrift
    ...
    Total_E+=-(math.atan(A_AJ/D_Tir)*1000/0.1)
