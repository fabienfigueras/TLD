...
def InitFromFile():
    global FNmontage,montage,FNlunette,FNrifle,rifle,FNamo,amo,FNbullet,bullet,FNbullet_BC,bullet_BC,FNzero,zero,atm_zero,FNenv,Lat_d,Lat_m,Lat_s,Lat_D,NS,Shoot_Atm,Sg_ICAO
...
#        Fx=float((-k/m)*abs(Vmoy)*Vx-2*(Oy*Vz-Oz*Vy))
        Fx=float((-k/m)*abs(Vmoy)*Vx-2*(-Oz*Vy))
    return(Fx)
...
def RK(D_Tir,h,MuzzleSpeed,NS,Az_d,Az,Co,m_gr,SightHeight,Twist,T_cal,Twist_C,Rifle_Brand,Rifle_Model,d_i,bl_cal,alpha_d,Alt,Gamma,Alt_ICAO,P_ICAO,T_C_ICAO,T_K_ICAO,RH_ICAO,Rho_ICAO,PVS_ICAO,PV_ICAO,TV_ICAO,P,T_C,T_K,RH,Rho,PVS,PV,TV,Omega,SoundSpeed_ICAO):
    ...
    global E_c,W_c,tph,Vxtph,Vytph,Vztph,Sg,Shoot_Wind,bullet,Lat_D,Vmod,Xtph,Ytph,Ztph,Arrow,Y_max,XY_max,TY_max,BC_Gx,BC_Go,Shoot_Wind
    ...
    #Shooting angle fron degre to RAD
...
def PRS_Solver(D_Tir,h,MuzzleSpeed,NS,Az_d,Az,Co,m_gr,SightHeight,Twist,T_cal,Twist_C,Rifle_Brand,Rifle_Model,d_i,bl_cal,alpha_d,Alt,Gamma,Alt_ICAO,P_ICAO,T_C_ICAO,T_K_ICAO,RH_ICAO,Rho_ICAO,PVS_ICAO,PV_ICAO,TV_ICAO,P,T_C,T_K,RH,Rho,PVS,PV,TV,Omega,SoundSpeed_ICAO):
    ...
    global H_Angle_d,H_Angle,Ytph,tph,Vmod,SD,Sg,Shoot_Wind,bullet,Ytph_HA
...
print("Shooting Direction (Azimut Angle) : ",Az_d," °",round(Az,5),"RAD")
...
print("Calculated Z Angle due to Wind Drift (mRAD) :",round(float(((Shoot_Wind.WDz)*1000)/D_Tir),2))
print("Time of Flight (s) :",round(tph,2))
...
print("Calculated bullet position (With only Drag and Gravity influences) ")
...
W_SpinDrift=-(A_SpinDrift*1000/D_Tir)/0.1
print("Windage correction due to Spin Drift (clicks) :",round(W_SpinDrift,0))
A_AJ=AJE
print("Aerodynamic Jump (m) :",round(A_AJ,3)," (cm) ",round(A_AJ*100,2))
print("Elevation correction due to Aerodynamic Jump (clicks) :",round(float((((-A_AJ)*1000)/D_Tir)/0.1),2))
...
Z_a=float((Shoot_Wind.WDz*1000)/D_Tir)
...
print("Aerodynamic Jump (m) :",round(AJE,2)," (cm) ",round(AJE*100,2))
...
print("Calculated Z shift due to Spin Drift (m) :",round(ZC_SpinDrift,2)," (cm) : ",round((ZC_SpinDrift*100),2))

print("Calculated Z Angle due to Spin Drift (mRAD) :",round((ZC_SpinDrift*1000/D_Tir),2))

print("Calculated Z shift due to Wind Drift and Spin Drift (m) :",round((Shoot_Wind.WDz+ZC_SpinDrift+Ztph),2)," (cm) :",round((Shoot_Wind.WDz*100+ZC_SpinDrift*100+Ztph*100),2))

Z_a=float((Shoot_Wind.WDz+ZC_SpinDrift+Ztph)*1000/D_Tir)
...
zero.alpha_d=zero.alpha_d+((-Y_a/1000)*(180/math.pi))
...
if H_Angle_d == 0:
    ...
    D_Tir_HA=D_Tir*math.cos(H_Angle)
    RK(D_Tir_HA,h,bullet.MuzzleSpeed,NS,Az_d,Az,Co,bullet.m_gr,montage.SightHeight,rifle.Twist,rifle.T_cal,rifle.Twist_C,rifle.Rifle_Brand,rifle.Rifle_Model,rifle.d_i,bullet.bl_cal,zero.alpha_d,Shoot_Atm.Alt,Gamma,ICAO_Atm.Alt,ICAO_Atm.P,ICAO_Atm.T_C,ICAO_Atm.T_K,ICAO_Atm.RH,ICAO_Atm.Rho,ICAO_Atm.PVS,ICAO_Atm.PV,ICAO_Atm.TV,Shoot_Atm.P,Shoot_Atm.T_C,Shoot_Atm.T_K,Shoot_Atm.RH,Shoot_Atm.Rho,Shoot_Atm.PVS,Shoot_Atm.PV,Shoot_Atm.TV,Omega,SoundSpeed_ICAO)
...
print("Elevation to be applied (clicks) +=>Up -=>Down:",round(E_c,0))
print("Windage to be applied (clicks) +=>Rigt -=>Left:",round(W_c,0))
...
if sys.argv[16] == "N":
    ...
    OutputFile.write("\t<tr>\n")
    Ligne400="\t\t<td align=\"center\" colspan=\"8\">"+" ALTITUDE (m) : "+str(Shoot_Atm.Alt)+" LATITUDE (°) : "+str(round(Lat_D,2))+" "+NS+" - Shooting Direction, relative to the North (°): "+str(Az_d)+"</td>\n"
    ...
    OutputFile.write("\t<tr>\n")
    Ligne6 = "\t\t<td align=\"center\">"+" TARGET DISTANCE (m) Include Gravity,Drag,Range Wind & Non-ICAO Influences"+"</td>"+"\t\t<td align=\"center\">"+str(int(D_Tir))+"</td>"+"\t\t<td align=\"center\">"+str(round((-math.atan(Y_Drop/D_Tir)*1000/0.1),0))+"</td>"+"\t\t<td align=\"center\" colspan=\"1\">"+" BULLET MAX HEIGHT (m) "+"</td>"+"\t\t<td align=\"center\">"+ str(round(Y_max,2))+"</td>"+"\t\t<td align=\"center\" colspan=\"1\">"+" @ DISTANCE (m) "+"</td>"+"\t\t<td align=\"center\">"+ str(round(XY_max,3))+"</td>"+"\t\t<td align=\"center\" colspan=\"1\">"+" N A "+"</td\n>"

    Total_E+=-round((math.atan(Y_Drop/D_Tir)*1000/0.1),1)
...
#    print("Drop Y with Angle :",Ytph_HA)
    if H_Angle_d==0:
        ...
        Delta_Y_Drop=-(Ytph_HA-Y_Drop)
#    print("Delta Y Drop :",Delta_Y_Drop)

    OutputFile.write("\t<tr>\n")
    Ligne7 = "\t\t<td align=\"center\">"+" SHOOTING ANGLE, relative to the horizontal (°) "+"</td>"+"\t\t<td align=\"center\">"+str(round(H_Angle_d,2))+"</td>"+"\t\t<td align=\"center\">"+str(round((math.atan(Delta_Y_Drop/D_Tir)*1000/0.1),0))+"</td>"+"\t\t<td align=\"center\" colspan=\"4\">"+" SPIN DRIFT "+"</td>"+"\t\t<td align=\"center\">"+ str(round(W_SpinDrift,0)) +"</td\n>"
    OutputFile.write(Ligne7)
    OutputFile.write("\t</tr>\n")

    Total_E+=round((math.atan(Delta_Y_Drop/D_Tir)*1000/0.1),1)
    ...
    OutputFile.write("\t<tr>\n")
    Ligne10 = "\t\t<td align=\"center\" colspan=\"2\">"+" RANGE WIND INFLUENCE "+"</td>"+"\t\t<td align=\"center\">"+" N A "+"</td>"+"\t\t<td align=\"center\" colspan=\"4\">"+" CROSS WIND INFLUENCE "+"</td>"+"\t\t<td align=\"center\">"+str(round((-math.atan(Shoot_Wind.WDz/D_Tir)*1000/0.1),0))+"</td\n>"
    OutputFile.write(Ligne10)
    OutputFile.write("\t</tr>\n")

    Total_W+=-(math.atan(Shoot_Wind.WDz/D_Tir)*1000/0.1)

    OutputFile.write("\t<tr>\n")
    Ligne11 = "\t\t<td align=\"center\" colspan=\"2\">"+" AERODYNAMIC JUMP (Due to Range Wind)"+"</td>"+"\t\t<td align=\"center\">"+str(round((-math.atan(A_AJ/D_Tir)*1000/0.1),0))+"</td>"+"\t\t<td align=\"center\" colspan=\"5\">"+"   "+"</td\n>"

    OutputFile.write(Ligne11)
    OutputFile.write("\t</tr>\n")

    Total_E+=round(-(math.atan(A_AJ/D_Tir)*1000/0.1),1)

    OutputFile.write("\t<tr>\n")
    Ligne12 = "\t\t<th colspan=\"2\">"+" TOTAL ELEVATION "+"</th>"+"\t\t<td align=\"center\">"+str(round(Total_E,0))+"</td>"+"\t\t<th colspan=\"4\">"+"TOTAL WINDAGE : "+"\t\t<td align=\"center\">"+str(round(Total_W,0))+"</th\n>"
...
#    print("Aerodynamic Jump (m) :",round(A_AJ,3)," (cm) ",round(A_AJ*100,2))
    A_AJ_R=-((A_AJ*1000/A_D_Tir_R)/0.1)
...
#    print("Spin Drift (m) : ",round(A_SpinDrift,3),"(cm): ",round((A_SpinDrift*100),2))
    ...
    A_SpinDrift_R=-((A_SpinDrift*1000/A_D_Tir_R)/0.1)
...
#    print("===================================")
    ...
    # Simulation for I / V hour
        ...
        Ligne200+="\t\t<td align=\"center\">"+"E: "+str(round(A_Delta_E,0))+" W: "+str(round(W_c,0))+" AJ: "+str(round(float((((-AJE)*1000)/D_Tir)/0.1),0))+"</td>\n"
